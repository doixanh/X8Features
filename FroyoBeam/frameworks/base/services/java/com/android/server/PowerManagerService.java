    ...
    private void setPowerState(int newState, boolean noChangeLights, int reason)
    {
        synchronized (mLocks) {
            ...
            if (mPowerState != newState) {
            	if (mPowerState == 3 && newState == 1) {
	            	Log.d(TAG, "dx: screen is being dimmed, skip now!");
	           	}
            	else 
				{
            ...
            if (ANIMATE_SCREEN_LIGHTS) {
                ...
                if ((newState & SCREEN_BRIGHT_BIT) == 0) {
                    ...
                    if ((newState & SCREEN_ON_BIT) == 0) {
                    	// dx : we are turning off the light
                    	isPoweringOff = true;
                ...
                try {
               	...
               	// dx : skip the bright animation when powering off the screen
               	if (!isPoweringOff) {
                	...
                	Log.i(TAG, "dx: skipping power off backlight animation.");
                }
            } else {
                if ((newState & SCREEN_BRIGHT_BIT) == 0) {
                    // dim or turn off backlight, depending on if the screen is on
